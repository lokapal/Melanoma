...
#  meanvect <- apply(alldata, 1, mean, trim=0.2) # trimmed mean
  ...
  medpkm   <- apply(FPdata, 1, median)        # median
...
write.table(as.data.frame(alldata), file="MP.hg38.TPM", row.names=F, col.names=T, sep="\t", quote=F)
write.table(as.data.frame(FPdata), file="MP.hg38.FPKM", row.names=F, col.names=T, sep="\t", quote=F)
write.table(as.data.frame(finalmean), file="MP.hg38.mean.TPM", row.names=F, col.names=T, sep="\t", quote=F)
write.table(as.data.frame(finalPKmean), file="MP.hg38.mean.FPKM", row.names=F, col.names=T, sep="\t", quote=F)
